...
const Input = forwardRef<HTMLInputElement, InputProps>(({ icon, error, label, variant = 'default', className = '', type, ...props }, ref) => {
  ...
  return (
    <div className={cn('w-full', styles.wrapper)}>
      {label && <label className={cn('mb-1.5 block text-sm font-medium', styles.label)}>{label}</label>}
