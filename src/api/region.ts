...
export const getAllRegencies = async (): Promise<Regency[]> => {
  const { data } = await server.get('/regions/regencies/all')

  return data
}

export const getAllDistricts = async (): Promise<District[]> => {
  const { data } = await server.get('/regions/districts/all')

  return data
}

export const getAllVillages = async (): Promise<Village[]> => {
  const { data } = await server.get('/regions/villages/all')
